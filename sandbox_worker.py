...
처리 흐름:
  1. S3에서 prompt를 받고, 현재 메시지 첨부는 Slack에서 직접 `/workspace/input/`에 내려받고
...
def _sqs_receive_one(queue_url: str) -> tuple[dict, str] | None:
    ...
    result = subprocess.run(
        [
            ...
            '1',
            '--wait-time-seconds',
            str(SQS_WAIT_TIME_SEC),
            '--visibility-timeout',
            str(SQS_VISIBILITY_TIMEOUT_SEC),
            '--output',
            'json',
        ],
        capture_output=True,
...
def download_input_files(input_files: list[dict]) -> list[str]:
    """현재 메시지 첨부를 Slack에서 직접 `/workspace/input/`에 내려받는다.

    각 항목은 봇이 SQS body에 담은 {filename, url, size}. Bearer 토큰으로 url을 받는다.
    SQS body는 신뢰 경계 밖이므로 filename을 한 번 더 sanitize해 경로 탈출을 막는다.
    """
    os.makedirs(INPUT_DIR, exist_ok=True)
    saved = []
    for f in input_files:
        ...
        if not filename or not url:
            continue
        safe_name = _sanitize_slack_attachment_filename(filename)
        dest = os.path.join(INPUT_DIR, safe_name)
        try:
            blob = _read_slack_private_url(url, os.environ['SLACK_BOT_TOKEN'], ARTIFACT_MAX_BYTES_PER_FILE)
            if not blob:
                continue
            ...
            logger.warning('Failed to download input %s', safe_name, exc_info=True)
    return saved
...
def process_job(job: dict, *, heartbeat=None, task_arn: str | None = None) -> None:
    ...
    download_input_files(input_files)
