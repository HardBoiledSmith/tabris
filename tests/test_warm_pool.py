...
def test_enqueue_claude_job_builds_body_and_group(monkeypatch):
    ...
    job_id = run_server._enqueue_claude_job(
        event, 'PROMPT BODY', '1700000000.000001', '1700000000.000100',
        [{'filename': 'a.txt', 's3_key': 'runs/TS/input/a.txt'}], received_at=1700000000.5,
    )
    ...
    assert body['input_files'] == [{'filename': 'a.txt', 's3_key': 'runs/TS/input/a.txt'}]
...
def test_stop_task_writes_cancel_marker_before_stop(monkeypatch):
    ...
    monkeypatch.setattr(run_server, '_s3_put_bytes',
                        lambda bucket, key, body, creds: seq.append(('marker', key)))
    monkeypatch.setattr(run_server, '_ecs_stop_task',
                        lambda arn, reason, creds: seq.append(('stop', arn)) or True)
    client = MagicMock()
...
def test_process_job_uses_arg_and_calls_heartbeat(monkeypatch):
    ...
    monkeypatch.setattr(sandbox_worker, '_self_task_arn',
                        lambda: (_ for _ in ()).throw(AssertionError('_self_task_arn should not be called')))

    hb = MagicMock()
    job = {
        'job_id': 'TS-MID', 'channel': 'D1', 'thread_ts': '1700000000.1',
        'waiting_msg_ts': '1700000000.2', 'user_id': 'U1',
        'prompt_s3_key': 'runs/x/prompt.txt', 'input_files': [], 'request_epoch': 1700000000.0,
    }
...
def test_main_loop_skips_when_cancel_marker_exists(monkeypatch):
    ...
    monkeypatch.setattr(sandbox_worker, 'process_job',
                        lambda job, **k: state['processed'].append(job['job_id']))
...
def test_main_loop_discards_message_without_job_id(monkeypatch):
    ...
    monkeypatch.setattr(sandbox_worker, 'MAX_JOBS', 5)
    monkeypatch.setattr(sandbox_worker, 'process_job',
                        lambda job, **k: state['processed'].append('x'))
