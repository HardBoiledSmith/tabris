...
샌드박스에 들어가는 스킬·`upload-artifact.sh`·`SKILL.md`·`CLAUDE.md` 변경은 **이미지를 재빌드·ECR 푸시**해야 반영된다. `run_create.sh`가 빌드·푸시·task def 갱신·서비스 업데이트를 한 번에 수행한다(이미지 변경 없이 인프라만 재적용할 땐 `SKIP_BUILD=1`):
