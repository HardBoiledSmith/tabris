...
* **`_provisioning/fargate/`**: SQS·ECS 클러스터/서비스·오토스케일·IAM·태스크 정의 생성/삭제(`run_create.sh`/`run_terminate.sh`).
...
* Run `cd _provisioning/fargate && AWS_PROFILE=<acct> ./run_create.sh` for 이미지 빌드·ECR 푸시 + SQS·ECS Service(워밍 풀)·오토스케일·IAM·task def 생성/갱신 + 서비스 업데이트를 한 번에 수행. 인프라만 재적용할 땐 `SKIP_BUILD=1`.
...
* 샌드박스(워커) 코드/스킬 변경은 봇 VM이 아니라 위 `run_create.sh`로 이미지를 재빌드·푸시해야 반영된다.
