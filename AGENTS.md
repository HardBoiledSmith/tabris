...
봇(`run_server.py`)은 Vagrant/EC2에서 상시 동작하며, 잡을 직접 실행하지 않고 **SQS FIFO 큐**에 적재한다. ECS Service로 떠 있는 워커(`sandbox_worker.py`)가 큐를 소비해 Claude를 실행하고 Slack에 결과를 게시한다(워밍 풀). `SQS_QUEUE_URL`이 비어 있으면 레거시로 봇이 ECS RunTask를 직접 호출한다(1회용, 롤백 안전망).

구성 요소는 대략 다음과 같다.

* **`run_server.py`**: Socket Mode 연결, `app_mention`·DM `message` 처리. 잡을 SQS FIFO에 적재(`MessageGroupId=user_id`로 유저별 직렬화) 또는 레거시 RunTask 호출. aws CLI subprocess로 S3/SQS/ECS 호출(boto3 미사용).
* **`sandbox_worker.py`**: Fargate 샌드박스 워커. 워밍 풀 모드에선 SQS long-poll 루프로 잡 처리, 잡 경계마다 workspace 초기화(격리), `MAX_JOBS`/`MAX_LIFETIME_SEC`마다 은퇴(ECS 교체). 멱등/취소는 S3 마커(`jobs/{job_id}/done`·`cancel`).
...
* **`_provisioning/fargate/`**: SQS·ECS 클러스터/서비스·오토스케일·IAM·태스크 정의 생성/삭제(`run_create_poc.sh`/`run_terminate_poc.sh`).
...
- **Sandbox**: ECS Fargate(Spot) — SQS FIFO 워밍 풀 또는 RunTask. AWS 호출은 boto3 대신 aws CLI subprocess.
...
* Run `cd _provisioning/fargate && AWS_PROFILE=<acct> ./run_create_poc.sh` for 이미지 빌드·ECR 푸시 + SQS·ECS Service(워밍 풀)·오토스케일·IAM·task def 생성/갱신 + 서비스 업데이트를 한 번에 수행. 인프라만 재적용할 땐 `SKIP_BUILD=1`.
* Run `cd _provisioning/fargate && AWS_PROFILE=<acct> ./run_terminate_poc.sh` for PoC 리소스(서비스·오토스케일·SQS·클러스터·IAM·workspace 버킷 등) 정리. 공유 버킷·ECR repo는 보존.
...
* 샌드박스(워커) 코드/스킬 변경은 봇 VM이 아니라 위 `run_create_poc.sh`로 이미지를 재빌드·푸시해야 반영된다.
...
| `tests/test_run_server_*.py` | 봇 단위(RunTask env, 입력 파일 S3 업로드, Slack 게시, usage 로깅) |
