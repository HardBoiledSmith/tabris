...
* **`Dockerfile` / `DOCKER_IMAGE`**: `my-claude-sandbox` 등 — `node:20-slim` 기반에 `@anthropic-ai/claude-code`를 올린 실행 환경
...
* VM에서는 `sudo systemctl {start|status|restart} tabris`, 로그는 `/var/log/tabris/bot.log` (README 「3. Vagrant VM 실행」)

#### Claude sandbox 이미지
* Run `docker build -t my-claude-sandbox .` for README에 적힌 이미지명·`DOCKER_IMAGE`와 동일한 태그로 빌드
...
* Run `vagrant ssh -c 'sudo docker build -t my-claude-sandbox /opt/tabris'` for Dockerfile·sandbox 이미지 갱신
