...
* **`run_server.py`**: Socket Mode 연결, `app_mention`·DM `message` 처리. 잡을 SQS FIFO에 적재(`MessageGroupId=user_id`로 유저별 직렬화, `MessageDeduplicationId=job_id`로 5분 중복 차단). aws CLI subprocess로 S3/SQS/ECS 호출(boto3 미사용). 첨부는 S3에 복사하지 않고 메타데이터(파일명·url)만 잡에 담고, 스레드 히스토리는 cursor 페이지네이션으로 모아 과거 첨부 목록을 프롬프트에 렌더한다.
* **`sandbox_worker.py`**: Fargate 샌드박스 워커. SQS long-poll 루프로 잡 처리, 잡 경계마다 workspace 초기화(격리), `MAX_JOBS`/`MAX_LIFETIME_SEC`마다 은퇴(ECS 교체). 현재 메시지 첨부는 Slack에서 직접 `/workspace/input/`에 받는다. 멱등/취소는 S3 마커(`jobs/{job_id}/done`·`cancel`). 메시지 수신~삭제 전 구간은 백그라운드 visibility 하트비트로 중복 재배달을 방지한다.
...
| `tests/test_sandbox_worker.py` | 워커 단위(claude 실행 파싱, 첨부 Slack 직접 다운로드, 이벤트 로깅 등) |
| `tests/test_run_server_*.py` | 봇 단위(아티팩트 게시, 첨부 메타 수집, 스레드 첨부 목록/페이지네이션, Slack 게시, usage 로깅) |
